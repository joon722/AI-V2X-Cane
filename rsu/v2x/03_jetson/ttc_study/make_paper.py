...
def build():
    ...
    story += [
        P("<b>초록</b> — 차량-보행자 충돌 위험 판단에 널리 쓰이는 충돌 예상 시간"
          ...
          "다만 <b>이는 TTC를 폐기하자는 주장이 아니다.</b> TTC가 매우 작을 때는 반박의 "
          "여지가 없는 위험 신호이므로, 본 연구는 물리적 근거로 산출한 안전 하한 2.0초를 "
          "판단 구조의 최하층에 배치하고 실기에 반영했다. TTC는 위험의 충분조건이 아니라 "
          "하한을 규정하는 신호로 쓰인다.", "abstract"),
        Spacer(1, 4),
    ...
    story += [
        ...
        P("<b>이 결론은 TTC의 폐기를 뜻하지 않는다.</b> TTC가 작다는 것은 여전히 반박할 "
          "수 없는 위험 신호이며, 본 연구도 안전 하한 2.0초를 TTC로 정의해 실기에 "
          "반영했다. 달라진 것은 TTC의 지위다. 위험을 판정하는 단일 기준에서, 위험의 "
          "하한을 규정하는 여러 신호 중 하나로 내려온 것이다. 실제로 치환 중요도에서 "
          "TTC는 네 번째였고 접근 속도가 4배 높았다.", "body"),

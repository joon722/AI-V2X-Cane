...
char webCarTelemetry[1500] = "";
char webCaneTelemetry[1300] = "";
...
uint8_t calculateRiskFromCane(const v2x_status_message_t &cane,
  ...
  *outInVehiclePath = false;
  ...
  // BOOT 영점보정과 최신 RSSI 근접 확인이 모두 있어야 CPA를 계산한다.
  ...
  *outHeadingError = headingError;
  ...
  if (filteredCaneRssiDbm < cfgRssiGateDbm) {
    return RISK_SAFE;
  }
  ...
  *outInVehiclePath = dCpa <= cfgCpaCautionM;
  ...
  if (dCpa <= cfgCpaDangerM &&
      tCpa <= RISK_DANGER_TTC_S) {
    return RISK_DANGER;
  }

  if (dCpa <= cfgCpaWarningM &&
      tCpa <= RISK_WARNING_TTC_S) {
    return RISK_WARNING;
  }

  if (dCpa <= cfgCpaCautionM &&
      tCpa <= RISK_CAUTION_TTC_S) {
    return RISK_CAUTION;
  }
...
void buildVehicleStatusPacket() {
  ...
  txVehicleStatus.rssi_distance_m = rssiEstimatedDistanceM;   // v4
}
